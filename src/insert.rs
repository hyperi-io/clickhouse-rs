...
impl<T> Insert<T> {
    ...
    /// # Errors
    ///
    /// Returns an error if the `Insert<T>` was constructed without
    /// validation (no `RowMetadata`), since the Native format
    /// requires per-column type information that only DESCRIBE
    /// TABLE provides.
    ///
    /// # Panics
    ///
    /// Panics if any rows have already been written via
    /// [`write`][Self::write] -- the format change must happen
    /// before the request body starts.
